...
import com.moxi.nexams.adapter.HistoryAdapter;
import com.moxi.nexams.cache.ACache;
import com.moxi.nexams.db.DownDbService;
            ...
            public void onItemClick(AdapterView<?> adapterView, View view, int i, long l) {
                try {
                    if (data.get(i).getType() == 0) {
                        ...
                        final int downIndex = data.get(i).getCobId();
                        ...
                        if (has != null && has.equals("0")) {
                            Intent paper = new Intent(MXHistoryActivity.this, MXPaperIndexActivity.class);
                            paper.putExtra("paper_index", data.get(i).getCobId());
                            paper.putExtra("paper_title", data.get(i).getTitle());
                            startActivity(paper);
                            return;
                        } else {
                            ...
                            OkHttpUtils.get().url(urlbase + downIndex + ".db").build().execute(new FileCallBack(Environment.getExternalStorageDirectory().getAbsolutePath(), "nexams/dbdata/" + downIndex + ".db") {
                                ...
                                public void onError(Call call, Exception e, int id) {
                                    ...
                                    Toastor.showToast(MXHistoryActivity.this, "下载失败");
                                ...
                                public void onResponse(File response, int id) {
                                    dialogShowOrHide(false, "请稍后...");
                                    Toastor.showToast(MXHistoryActivity.this, "下载成功");
