 ...
 */
public class WritePadCommonBaseView extends View implements PenReader.PenReaderCallback {
    public WritePadCommonBaseView(Context context) {
        super(context);
        initView();
    }

    public WritePadCommonBaseView(Context context, AttributeSet attrs) {
        super(context, attrs);
        initView();
     ...
     * 手指区别
     */
    public boolean fingerDistinction=false;
    /**
     * Y轴移动距离
     */
...
//
//    }
    public void setScroolToY(int y){
        this.scroolToY=y;
    }
            ...
            if (bitmap.isRecycled() && codeAndIndex != null) {
                ...
                DbPhotoLoader.getInstance().loaderPhoto(codeAndIndex.saveCode, codeAndIndex.index, new DbPhotoListener() {
                    ...
                    public void onLoaderSucess(String saveCode, int index, Bitmap bitmap) {
                        WritePadCommonBaseView.this.bitmap = bitmap;
                    }
                });
            }
...
//    public void setUpdateViewMatrixY(int y){
//        int viewPosition[] = {0, 0};
//        getLocationOnScreen(viewPosition);
//        viewPosition[1]=y;
//        viewMatrix = new Matrix();
...
//    }
    ...
    public void onEndRawData() {
        if (DrawIndex==-1){
            ...
            isDown = false;
            EpdController.leaveScribbleMode(WritePadCommonBaseView.this);
        }
    }
    ...
    public void onRawTouchPointListReceived(TouchPointList touchPointList) {
        ...
        for (TouchPoint touchPoint : touchPointList.getPoints()) {
            ...
            if (begin) {
                ...
                float dst[] = mapPoint(point.getX(), point.getY()+scroolToY);
                EpdController.startStroke(paintSize, dst[0], dst[1], pressure, size, System.currentTimeMillis());
                path.moveTo(eventX, eventY+scroolToY);
            } else {
                if (screenY!=0&&eventY>screenY)eventY=screenY;
                float dst[] = mapPoint(point.getX(), eventY+scroolToY);
                ...
                try {
                    path.lineTo(eventX, eventY + scroolToY);
                }catch (Exception e){
                    e.printStackTrace();
                }
//                EpdController.addStrokePoint(paintSize, dst[0], dst[1], pressure, size, System.currentTimeMillis());
    ...
    public void onBeginErasing() {
        ...
        nibWipe = true;
        isDraaw = false;
        paintClear.setAlpha(255);
        paintClear.setColor(Color.WHITE);
        paintClear.setXfermode(null);
    }
