...
public class GestrueControlGalleryView extends ViewGroup implements
		ClockCallback {
	...
	public GestrueControlGalleryView(Context context, AttributeSet attrs) {
		super(context, attrs);
		init(context);
	}
	...
	private void init(Context context) {
		...
		EXTRA_SCROLL_DIS = Utils.dip2px(getContext(), 80);
	}
	...
	private void sendPageChangeBroadcast() {
		if (mGalleryId >= 0) {
			...
			intent.putExtra(GalleryViewActivity.KEY_GALLERY_PAGEINDEX,
					mCurrentPage);
			intent.putExtra(GalleryViewActivity.KEY_GALLERY_ID, mGalleryId);
	...
	private void calculateDisTranslateY() {
		float scaleWidth = (float) mWidth / mGallaryData.getImageRect().width();
		float height = mGallaryData.getImageRect().height() * scaleWidth;
	...
	public void snapToPage(int whichPage) {
		...
		if (isChange && mPageChangeListener != null) {
			mPageChangeListener.onPageChange(mGallaryData, mGalleryId,
					mCurrentPage);
		}
